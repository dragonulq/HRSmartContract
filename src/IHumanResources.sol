...
/// notice This interface defines the functions that the HumanResources contract must
//implement
/// The contract must be able to register employees , terminate them , and allow
//employees to withdraw their salary
/// The contract will be funded using only USDC but will pay the employees in USDC or
//ETH
interface IHumanResources {
/// notice This error is raised if a user tries to call a function they are not
//authorized to call
error NotAuthorized () ;
/// notice This error is raised if a user tries to register an employee that is
//already registered
error EmployeeAlreadyRegistered () ;
/// notice This error is raised if a user tries to terminate an employee that is
//not registered
...
/// param amount must be the amount in the currency the employee prefers ( USDC or
//ETH ) scaled correctly
event SalaryWithdrawn ( address indexed employee , bool isEth , uint256 amount );
/// notice This event is emitted when an employee switches the currency in which
//they receive the salary
event CurrencySwitched ( address indexed employee , bool isEth );
/// HR manager functions
/// Only the address returned by the ‘hrManager ‘ below is able to call these
//functions
/// If anyone else tries to call them , the transaction must revert with the ‘
//NotAuthorized ‘ error above 
...
/// If anyone else tries to call them , the transaction shall revert with the ‘
//NotAuthorized ‘ error above
/// Only the ‘withdrawSalary ‘ can be called by non - active (i.e. terminated )
//employees
/// Withdraws the salary of the employee
/// This sends either USDC or ETH to the employee , depending on the employee ’s
//preference
/// The salary accumulates with time ( regardless of nights , weekends , and other
//non working hours ) according to the employee weekly salary
/// This means that after 2 days , the employee will be able to withdraw 2/7 th of
//his weekly salary
...
/// When the salary is paid in ETH , the contract will swap the amount to be paid
//from USDC to ETH
/// When this function is called , the current accumulated salary should be
//withdrawn automatically ( emitting the ‘SalaryWithdrawn ‘ event )
...
/// If the employee does not exist , the function does not revert but all values
//returned must be 0
/// param employee the address of the employee
/// return weeklyUsdSalary the weekly salary of the employee in USD , scaled with
//18 decimals
/// return employedSince the timestamp at which the employee was registered
/// return terminatedAt the timestamp at which the employee was terminated (or 0
